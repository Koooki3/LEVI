...
## Local model state (unreleased)

`outputs/LEVI/workbench/agent/workbench.sqlite3` also stores model manifests,
download jobs, requests and annotation-phase teaching records. These are durable
records, not cleanup caches. A dedicated Ollama instance uses
`outputs/LEVI/workbench/models/ollama/` for its process identity/home/log and
`checkpoints/ollama/` for model storage. Ollama's internal content-addressed blobs
are an explicit naming exception; do not rename them. Existing external service
paths are not inferred or managed. See [local model setup](docs/OLLAMA.md).
