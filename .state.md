...
| `.cache/`, `tmp/` | HF Hub cache (`org__name@<revision>--<account scope>`), build/test temp files. | `levi clean` covers this. |
...
- Documented exceptions: the Hub cache keeps a one-way digest of the HF account (`--<scope>`) so accounts never share private files; transient `.tmp` files and content fingerprints are hashes by nature.
