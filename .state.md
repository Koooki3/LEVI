...
- **One per run** — jobs, conversion outputs, SAM3 plans/jobs/revisions, a clashing export: a timestamp `YYYYmmdd-HHMMSS-fff`, reserved atomically (`O_EXCL` / `mkdir`) so concurrent processes never collide (`-1`, `-2`… on the same millisecond).
