...
4. **Remaining episodes.** Completed episodes are never redone; human edits are kept. For large scopes call `runs.prepare` with `episodes: [..]` in batches of at most 16 after pilot review, then propose only those inspected episodes. The bounded call avoids the Core request timeout and prepared episodes accumulate across calls. An explicit batch cannot reprepare a completed episode or exceed the approved scope.
