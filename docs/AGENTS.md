...
1. **Plan.** In **Agent Workbench → Tasks & review**, choose episodes (click or drag), cameras, a task type — dataset review, video subtasks and events, or visible object masks — and instructions. For subtask work, write each subtask's definition: when it starts, when it ends, what counts as success. Those definitions are the contract the agent annotates against. A dataset review samples `samples_per_episode` frames spread over each episode; set the workflow's `final_samples` to put that many of them on the episode's end (0.5 s apart, ending on the last frame) when the outcome is judged on the final state. A temporal plan's `refine` (`always`, the default and recommended, or `auto`) decides whether a model LEVI runs refines its draft's boundaries in a second pass; `auto` skips it when the coarse step is at most half the boundary window, which saves about half the calls but costs accuracy (in paired runs `always` scored higher on segment F1 and on outcomes).
...
3. **Pilot.** One episode first — unless the plan sets `require_human_pilot: false`, which the person approving the plan accepts along with it (plan approval and the final commit still take a person). On the MCP channel the agent picks the run up itself: `runs.list` shows every run in its scope and whose turn it is — `human_approval → agent_prepare → agent_propose → human_review → human_commit`. A person reviews the pilot and accepts or rejects it; a rejected pilot blocks the rest.
4. **Remaining episodes.** Completed episodes are never redone; human edits are kept. An external agent's first `evidence.read` of an episode prepares it, so no separate step is needed. To prepare ahead, call `runs.prepare` with `episodes: [..]` in batches of at most 16 (after pilot review), then propose only those inspected episodes; the bounded call avoids the Core request timeout and prepared episodes accumulate across calls. An explicit batch cannot re-prepare a completed episode or exceed the approved scope, and an episode already prepared is kept as it is (its refined frames stay).
...
- `evidence.read` with `layout: "mosaic"` returns one labelled contact sheet (six frames to a row, up to 48 frames) instead of one image per frame, so an episode is usually one call; the answer lists the frame times, and `ids: true` adds each frame's evidence id. `images: false` returns the ids and times only. An external agent's plan samples one frame per second by default (a model run keeps two seconds). The first read of an episode prepares its evidence, so `runs.prepare` is optional.
- The first page of `evidence.read` also carries `signals`: a few lines of what the dataset's own state/action columns record — gripper close/open (and, for a continuous gripper, how far it closed as a share of the dataset's range), the arm's height turns and its still spans, to the frame (`levi/agent/signals.py`). Only declared float vector columns and their dimension names are used; datasets without them get no signals, and `signals: false` leaves them out. They record what the robot did and are given without interpretation — neither boundaries nor attempts: against a reference annotation, releases sat on the recorded opening while contact and lift did not sit on the closing or the lowest point, and agents told to count attempts with them split repeated grasps the task counted as one. `episodes: [a, b, …]` (up to four, whole episodes as mosaics) reads several episodes in one call and lists their sheets in `mosaics` — fewer agent turns, each of which re-reads the agent's context.
- `evidence.refine` adds dense frames only around moments the coarse pass could not resolve, within the plan's window and frame cap, and answers with one JPEG sheet of the added frames of the call (six to a row, split past 30) and one summary line per instant. Refinement samples at the plan's boundary tolerance; `window_seconds` narrows the window around each instant (never wider than the plan's). `ranges: [[from, to]]` (up to six spans of at most 10 s) watches a stretch at `step_seconds` — what a reader of the video does when a sheet leaves a boundary or an outcome unsettled — and `episodes: {N: {...}}` refines up to four episodes, each with its own instants and spans, in one call. When the cap cannot take every requested instant, the ones that fit are refined and the rest are listed in `skipped_around_seconds`.
- `annotations.propose_segments` takes several episodes per call, either as `proposals` or in the annotator's own shape, `segments: {"<episode>": [{"start", "end", "subtask", "outcome", "description"}]}`. An external agent may leave `evidence_ids` out (LEVI cites the observed frames inside each interval), a `success` uses its content as the evidence note unless one is given, and a boundary within half a frame of the episode's first or last frame is snapped to it. A model's own answers are not completed this way: they must cite their evidence.
- **Subtask vocabulary.** A temporal plan without definitions gets the dataset's vocabulary in force: its own `vocabulary.json`, else LEVI's built-in manipulation vocabulary `levi/knowledge/subtasks.json` (object-transfer phases approach, grasp, transport, place, retreat, then contact skills push, pull, open, close, rotate, screw, press, insert, remove, pour, scoop, stir, cut, wipe, fold, reorient, hold, handover — each with its observable start, end and success, confusions and the aliases other datasets use; sources: RH20T-P, AgiBot World, BridgeData V2, DROID, EPIC-KITCHENS-100, MimicGen, Rubicon Boundaries). An annotator adds a subtask the vocabulary lacks with `new_subtasks` on `annotations.propose_segments` (id, definition, starts_when, ends_when, success_when; an existing id or alias is refused with the entry to use); it holds for the rest of the run, and when a person commits, the additions the committed segments use join the dataset's vocabulary with `source: agent` and the run that added them.
- `annotations.propose_segments` answers with a receipt that names `problems` only for an episode whose staged segments break a general rule (`levi/agent/checks.py`): two intervals of one subtask meeting (one attempt, or an approach is missing between two), an `unknown` outcome directly followed by a segment with a known outcome (what comes next settles it; `other` — a human hand, built-in rule annotation-004 — and classes whose plan says the outcome is always unknown are exempt). A clean staging gets no line. Measured on the full-dataset comparison of 2026-09-23, a routine boundary check of every staged episode was used 20 times and led to one re-staging; the problem lines replace it. Measured against a full-dataset reference of 2026-09-24, the `unknown` lines were right 78–90% of the time; a check that read two attempts off the recorded gripper opening and the arm rising was right about half the time and was dropped — whether the effector left the object is for the frames to show.
- `evidence.boundaries` is the look for an episode a problem line names (or any doubt): for up to four staged episodes, one sheet row per boundary between consecutive segments (seven frames from 1 s before to 1 s after it, the middle tile being the boundary) and the plan's `starts_when` / `ends_when` of the subtasks involved. An episode that needs it is re-staged with `annotations.propose_segments` and `replace: true`, which replaces that episode's segments in place and never touches episodes a person has reviewed.
...
| | `runs.abandon` | **person** |
| Evidence | `evidence.read`, `evidence.refine`, `evidence.boundaries`, `evidence.changes`, `media.sample`, `view.seek` | agent |
