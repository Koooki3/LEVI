...
1. **Plan.** In **Agent Workbench → Tasks & review**, choose episodes (click or drag), cameras, a task type — dataset review, video subtasks and events, or visible object masks — and instructions. For subtask work, write each subtask's definition: when it starts, when it ends, what counts as success. Those definitions are the contract the agent annotates against. A dataset review samples `samples_per_episode` frames spread over each episode; set the workflow's `final_samples` to put that many of them on the episode's end (0.5 s apart, ending on the last frame) when the outcome is judged on the final state. A temporal plan's `refine` (`always` by default, `auto`, `never`) decides whether a model LEVI runs refines its draft's boundaries in a second pass; `auto` skips it when the coarse step is at most half the boundary window, where the dense first pass already shows every boundary.
