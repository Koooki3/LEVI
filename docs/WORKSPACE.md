...
### Namespaces: one input, many experiments

Several independent experiments (annotation modes, rounds, teachers) can use ONE input dataset without copying it and without any of them seeing another's results. `levi namespace create <dataset> <namespace>` (or `POST /api/levi/catalog/<dataset>/namespaces`) registers `<dataset>--<namespace>`: a catalog entry of its own over the same source folder and browsing view (`base` and `namespace` fields; the source is never copied). Everything LEVI keys by catalog name is therefore separate per namespace — annotation sidecars and outcome labels, reviews, exports, agent runs, revisions and the active head (so baselines never collide), dataset memory, teaching and examples given to models, eval records and cost profiles — and a namespace's notes never become built-in knowledge candidates. Path lookups (a folder, a view) always answer the base dataset; a namespace follows its base's view and is unregistered with it (its files stay). A namespace is 1–64 letters, digits, `.`, `_` or single `-`. `levi namespace list [dataset]` and `levi namespace remove <dataset>--<namespace>` (files stay) complete the set.
...
- **Selection** follows the reference subset script: every episode's `metadata_*.json` is listed, grouped by (lab, outcome), and ordered round-robin with seed 42 — no language or success filter. A new workspace's first draw starts at position 0 of that order (the script's `selected_500`). `levi sample draw` (or `POST /api/levi/samples/droid_raw`, `size` 1–5000) makes another draw, `droid_raw_<size>_draw02/` and so on, from the next unused positions: the draws of one workspace never share an episode, also when its ledger was lost (the cursor is recovered from the `order_positions` of the sample folders in place, and a complete one is recorded as a ready draw). An episode LEVI's DROID reader rejects is replaced by the next one of the order.
- **What a draw is**: its positions in that one order, recorded with every replaced episode in `_meta/selection.json` (`order_positions`, `replaced`) and listed episode by episode in `_meta/selected.tsv` — not its name. A draw starts where the workspace's earlier draws ended, so the same name holds the same episodes on two machines only when both made the same draws before it (same sizes, nothing discarded) and replaced the same episodes: a replacement after download depends on LEVI's reader, and without the `droid` extra the trajectory is not checked (`trajectory_checked: false`). Compare samples across machines by `order_positions` and `selected.tsv`.
- **Files kept**: `metadata_*.json`, `trajectory.h5` and the non-stereo `recordings/MP4/*.mp4`; no SVO, no stereo MP4. Every file is checked against the bucket's size and MD5 before it counts. Measured on the first draw (2026-09-24): 500 episodes, 11.60 GiB verified (about 24 MiB per episode); 13 of the 513 candidates it went through were replaced for having fewer than 3 non-stereo MP4 cameras.
- **Space** is not checked before a draw. `levi sample status` shows the disk's free space and each draw's planned bytes.
- **Network**: each listing page and each file is one request, retried as a whole up to 6 times with backoff; a refusal (400/401/403/404) and a local disk error (full disk, quota, file too large, read-only) are not retried.
- **Stopping**: Ctrl-C stops a foreground `levi sample draw` within a chunk of the files in flight; the draw lets go of its lock only once its download threads have stopped, and records `interrupted`. `levi sample cancel` stops a background worker LEVI started, or a foreground `levi sample draw` whose recorded process identity (start time, boot, executable) it can check — a draw it cannot identify is left alone and the cancel is refused (API: 409) without changing anything. The partial folder stays; `levi sample cancel --discard` (API: `?discard=true`) deletes it, only once the draw has stopped, and closes the draw; the next draw then takes the same positions of the order.
- **Resuming**: the next start (`levi sample draw`, the API) fetches only what is missing, and an unfinished draw is always resumed before a new one is planned. When the core starts it resumes by itself a draw left interrupted (the core, a Ctrl-C or a kill stopped it) or one that failed on the network, at most 3 automatic attempts per draw; a draw that was cancelled or failed for another reason (a refusal, a full disk, a failed verification) waits for `levi sample draw`. A draw whose folder was already published when its process stopped is recorded ready by the next start, not downloaded again. The automatic first draw happens once, and only if no draw was made by hand first and no complete sample folder is in place.
- **Settings**: `LEVI_DROID_SAMPLE=off` (also `0`, `false`, `no`) means LEVI never starts or resumes a download by itself (tests and CI set it; drawing by hand still works); `auto`, `on`, `1`, `true`, `yes`, empty or unset enable it, and any other value keeps it off with a warning in the log. `LEVI_DROID_SAMPLE_WORKERS` (1–8, default 4) sets parallel episode downloads.
- `levi sample status` shows the ledger, progress (bytes, replacements, time left from the rate since the downloads began) and free disk space; `levi sample draw [--size N] [--workers W]` (`--workers` 1–8) draws in the foreground with progress; `levi sample cancel [--discard]` stops a draw. `GET /api/levi/samples`, `POST /api/levi/samples/droid_raw` and `POST /api/levi/samples/droid_raw/cancel` are the same for the web UI ([API](API.md)).
