...
def test_compatible_provider_uses_pinned_transport_and_typed_output(monkeypatch,tmp_path):
    pytest.importorskip('pydantic_ai')
    from openai import AsyncOpenAI

    from levi.agent import providers
    from levi.agent.schema import Budget, ProviderConfig
    module='httpx2' if 'httpx2' in str(inspect.signature(AsyncOpenAI).parameters['http_client'].annotation) else 'httpx'
    http=importlib.import_module(module)
    observed=[]
    async def respond(self,request):
        payload=json.loads(request.content)
        observed.append((request,payload))
        name=next(t['function']['name'] for t in payload['tools'] if 'proposals' in t['function']['parameters'].get('properties',{}))
        return http.Response(200,json={'id':'fixture','object':'chat.completion','created':1,'model':'fixture','choices':[{'index':0,'finish_reason':'tool_calls','message':{'role':'assistant','content':None,'tool_calls':[{'id':'result','type':'function','function':{'name':name,'arguments':json.dumps({'summary':'Offline fixture','proposals':[],'warnings':[]})}}]}}],'usage':{'prompt_tokens':10,'completion_tokens':20,'total_tokens':30}},request=request)
    ...
    output,usage=providers.CompatibleProvider().generate(ProviderConfig(name='fixture',base_url='https://example.invalid/v1',model='fixture',key_env='LEVI_TEST_KEY',tools=True),
        'Inspect',{'episode_index':0},[],tmp_path,Budget())
...
def test_mcp_server_builds_offline():
    ...
    server=build_server()
    for request in (types.ListToolsRequest,types.CallToolRequest,types.ListResourcesRequest,types.ReadResourceRequest,types.ListPromptsRequest,types.GetPromptRequest):
...
def test_mcp_roundtrip_preserves_scope_and_exposes_skills(monkeypatch,tmp_path):
    ...
    def request(path,payload=None):
        if path=='capabilities':return invoke(wb,who,'capabilities.list',{})
        return invoke(wb,who,payload['name'],payload['arguments'])
    monkeypatch.setattr(bridge,'request',request)
    async def check():
        async with create_connected_server_and_client_session(bridge.build_server()) as client:
