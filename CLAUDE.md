...
- `levi/inference/`: native Ollama provider, owned service and transport. `gpu.py` is the off-peak guard: no local-model call or service start while another process computes on the GPU or left it < `LEVI_GPU_QUIET_SECONDS`; the service unloads models when one appears. Tests set `LEVI_GPU_SHARING=allow` in `conftest.py` so results never depend on who is training on the machine.
