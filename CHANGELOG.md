...
- **A small model is fenced in by its schema.** Per-request output schemas allow only the plan's kinds and subtasks and the evidence ids actually shown; intervals must carry an end, an outcome and an evidence note; refinements keep the draft's intervals one for one, and boundaries moved beyond the frames shown keep the draft's. Invalid answers keep their valid proposals (for the teacher, or dropped and recorded without one), and their tokens are always settled; an episode with no valid answer at all is set aside and the run goes on (the pilot, or more than 10 % set aside, still blocks). A review can put `final_samples` of its frames on the episode's final state. A temporal interval's subtask can no longer be decoded as null.
- **Local OpenAI-compatible servers (vLLM).** Provider kind `openai-local` runs a model served by vLLM through the same hardened path as Ollama: narrowed `json_schema` answers with thinking off, compact evidence, request-cost calibration, `max_images` and `image_max_side`, salvage, pause/cancel, natural-language tasks and the `local-vlm` evaluation driver. `fold_system` serves templates without a system role (Molmo2). Binding sets `context_tokens` to the server's `--max-model-len`. The GPU guardian recognises the server a profile points at by its listening socket, and resumes a blocked run only when the GPU is clear for that run's own profile. The guard now also covers every loopback spelling of a local endpoint (`127.0.1.1`, the host name, `[0:…:1]`). Ollama calls now keep `load_seconds`, `prefill_seconds` and `decode_seconds` in their usage.
...
- **Built-in subtask vocabulary.** Every dataset without its own vocabulary uses LEVI's task-agnostic manipulation vocabulary (`levi/knowledge/subtasks.json`, 23 subtasks: object-transfer phases plus contact skills, each with observable start/end/success, confusions and aliases, from RH20T-P, AgiBot World, BridgeData V2, DROID, EPIC-KITCHENS-100, MimicGen, Rubicon Boundaries). A temporal plan without definitions gets it; annotators add missing subtasks with `new_subtasks` (never a synonym of an existing id or alias), and a committed addition joins the dataset's vocabulary. Plans hold up to 64 definitions. `temporal-annotation` skill @7.
